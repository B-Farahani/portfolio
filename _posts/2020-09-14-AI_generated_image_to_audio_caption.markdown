...
This was just an example for a single caption. Each caption will have a "triangle" like this with their relative numbers. The numbers shown above are just made-up examples, but the numbers will be between 0 and 1  because they are probability values.  
...
Blow are a few examples of the captions generated by the model. I have converted them to speech and will add the audio here soon!
...
**Generated caption:**    "a man in a red shirt and helmet is riding a bike on a dirt path"

# Conclusions 

I was able to build a decent model and generate some great audio captions using deep learning and only 6,000 images and captions. The model was strengthened by the power of the transfer learning (InceptionV3 for images and GloVe for captions), where the models were previously trained on very large image and text datasets. To build a fully deployable technology to aid people with visual impairments, we need to train a model on a huge amount of data containing many image classes. That's not easy, but perhaps we can start small, by training a model only objects inside a building (e.g. a new office) or on the street (e.g. to cross or navigate a street).
