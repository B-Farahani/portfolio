...
I was able to build a decent model and generate some great audio captions using deep learning and only 6,000 images and captions. The model was strengthened by the power of transfer learning (InceptionV3 for images and GloVe for captions), where the models were previously trained on very large image and text datasets. To build a fully deployable technology to aid people with visual impairments, we need to train a model on a huge amount of data, containing many image classes. That is not easy and needs a lot of resources, but perhaps we can start small, by focusing on the objects inside a building (e.g. to help navigating through a new office environment) or on the street (e.g. to help safely crossing a street).
