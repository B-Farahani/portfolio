...
We were able to build a decent model and generate some great captions with training a neural network model on only 6,000 images and captions. The model was strengthened by the power of the transfer learning (InceptionV3 for images and GloVe for captions), where the models were previously trained on very large image and text datasets. It should be noted that to generate captions for new images, the images should be semantically related to the training images. For example, if we only train the model on cats and dogs, the model can only predict cats or dogs and not fruits or flowers! 
