...
I was able to build a decent model and generate some great audio captions using deep learning and only 6,000 images and captions. The model was strengthened by the power of transfer learning (InceptionV3 for images and GloVe for captions), where the models were previously trained on a large image and text dataset. To build a fully deployable technology to aid people with visual impairments, we need to train a model on an enormous amount of data, containing many image classes. That is not easy and requires a lot of resources, but we can start small by focusing on the objects inside a building (e.g. to help to navigate through a new office environment) or on the street (e.g. to help to safely cross a street).
